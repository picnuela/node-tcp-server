...
/**
 * Gets Map's value searching all specified keys.
 * @returns {String} value for one of specified keys or empty string.
 */
FnUtil.prototype.getMapString = function () {
  let fn = this;
  let sval = fn.getKey(arguments[0], arguments[1]);
  if (!sval) sval = "";
  sval = fn.trim(sval);
  return sval;
}
...
/**
 * Checks if object is an Stream Server.
 * @returns true if object is an Stream Server.
 */
FnUtil.prototype.isServer = function () {
  let fn = this;
  let obj = arguments[0];
  if (obj) {
    if (obj.parent) obj = obj.parent;
    return fn.isFunction(obj.listen);
  }
  return false;
}
/**
 * Decode received Text.<br>
 * Sequences '\r' will be represented as lineFeed an so on.<br>
 * From JSon text '/r', '/n' sequences was found.
 * @returns {String}
 */
FnUtil.prototype.textDecode = function () {
  let fn = this;
  let txt = fn.trim(arguments[0]);
  let rsp = "";
  let ix = 0;
  while (ix < txt.length) {
    let c0 = txt.charAt(ix++);
    if (c0 == '/' || c0 == '\\') {
      if ((txt.length - ix) < 1) continue;
      let c1 = txt.charAt(ix++);
      switch (c1) {
        case 'n':
          c0 = '\n';
          break;
